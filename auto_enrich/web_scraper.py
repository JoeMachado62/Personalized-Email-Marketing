"""
Enhanced web scraper that gathers data BEFORE sending to AI.
...
# Use Selenium with real Chrome browser for reliable search results
try:
    from .search_with_selenium import search_with_real_chrome
    logger.info("Using Selenium with real Chrome browser")
    
    async def search_web(query: str) -> dict:
        """Wrapper to make Selenium search async-compatible"""
        import asyncio
        loop = asyncio.get_event_loop()
        results = await loop.run_in_executor(None, search_with_real_chrome, query, False)  # headless=False for better results
        return {
            'query': query,
            'results': results,
    ...
    from .search_engines import search_web
    logger.info("Falling back to standard search engines")
